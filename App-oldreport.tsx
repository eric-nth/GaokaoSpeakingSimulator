...
import { Play, Mic, Clock, CheckCircle, ChevronRight, FileAudio, AlertCircle, Download, RefreshCw, Loader2, FolderOpen, BookOpen, Sparkles, BrainCircuit, Flag, Printer, Key, Save } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const saveApiKey = () => {
    if (apiKey.trim().length > 0) {
      // Basic validation could be done here or just try to use it
  ...
  const handleGrade = async (question: Question, sectionId: string, blob: Blob) => {
    ...
    try {
      ...
      if (sectionId === 'sec_1' || sectionId === 'sec_2') {
        prompt += `
          Task: Read Aloud.
          Original Text: "${question.promptText}"
          ...
          Output: Score (out of 100) and concise feedback in Chinese.
        `;
      ...
      // --- SECTION 3: SITUATION Q&A ---
        ...
        // Only provide the FIRST reference answer to AI (as per instruction, but we pass valid options)
        const refAnswer = gradingContext.refAnswers ? gradingContext.refAnswers[0] : question.answerContent;
        
        prompt += `
          Task: Situation Q&A. The student asks questions based on a prompt.
          Prompt Info: "${question.promptText}"
      ...
      // --- SECTION 4: PICTURE DESCRIPTION ---
        ...
        prompt += `
          ...
          Requirements:
          1. Start with the sentence: "${question.promptText}" (The student must say this or connect to it).
      ...
      // --- SECTION 5: FAST RESPONSE ---
        ...
        // DO NOT PASS VIDEO. Pass actual question text.
        const actualQ = gradingContext.actualQuestion || "Unknown Question";
        
        prompt += `
          Task: Fast Response.
          Question asked to student: "${actualQ}"
          Reference Answer: "${question.answerContent}"
      ...
      // --- SECTION 6: SUMMARY & Q&A ---
        ...
        // DO NOT PASS AUDIO (Original Article).
        const originalText = gradingContext.originalText || "";
  ...
  const calculateTotalScore = () => {
    ...
    Object.keys(gradingResults).forEach(qId => {
       ...
       const maxPoints = SECTION_POINTS[qId] || 0;
       // result.score is out of 100. Scale it to maxPoints.
       const weightedScore = (result.score / 100) * maxPoints;
       total += weightedScore;
    });
          ...
          {examData.map((section, idx) => {
            ...
            return (
              <div key={section.id} className="mb-2">
                <div className={`px-4 py-3 flex items-center justify-between font-semibold text-sm ${isActiveSection ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-600' : 'text-gray-600'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm print:hidden">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
               ...
               <p className="text-sm text-gray-500 mb-4">
                 AI grading requires a valid Google Gemini API Key. Keys are stored locally in your browser.
               </p>
               <input 
                 type="password" 
                 value={apiKey} 
                 ...
                 placeholder="sk-..."
                 className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none mb-2 font-mono text-sm"
               />
               <a 
                 href="https://aistudio.google.com/app/apikey" 
                 target="_blank" 
                 rel="noreferrer"
                 className="text-xs text-blue-600 hover:underline mb-6 block"
               >
               ...
               <div className="flex justify-end gap-3">
                 ...
                 <button onClick={saveApiKey} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Save Key</button>
               </div>
            </div>
          </div>
        )}
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-baseline justify-between mb-1">
                                    ...
                                    <span className="text-xl font-black text-purple-600">{grade.score}<span className="text-xs text-purple-400 font-medium ml-1">/100</span></span>
                                  </div>
                         ...
                         <div className="mt-4 pt-4 border-t border-dashed border-gray-200 print:hidden">
                            ...
                            <div className="bg-green-50/50 p-3 rounded-md border border-green-100 text-sm text-gray-700 leading-relaxed">
                               {q.answerType === 'audio' ? (
                                 ...
                                 <p className="whitespace-pre-wrap font-serif">{q.answerContent}</p>
                               )}
                            </div>
                         </div>
                      </div>
              ...
              <div className="w-full h-16 flex items-center justify-center mb-2">
                 {(showRecord || isFinishing) ? (
                   ...
                   <div className="text-gray-400 font-medium flex items-center gap-2">
                     {showPrep ? 'Prepare your answer...' : isStarting ? 'Wait for signal...' : ''}
                   </div>
                 )}
              </div>
